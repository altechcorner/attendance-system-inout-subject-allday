          ...
          if (attendanceRows.length === 0 || attendanceRows[0].time_out !== null) {
            ...
            db.query(
              ...
              async (err) => {
                ...
                const htmlMessage = `<p>Hello ${student.name},</p><p>You have <b>timed IN</b> at <b>${now.toLocaleString()}</b>.</p><br><strong>Active Subject:</strong> ${activeSubject}`;
            ...
            db.query(
              ...
              async (err) => {
                ...
                const htmlMessage = `<p>Hello ${student.name},</p><p>You have <b>timed OUT</b> at <b>${now.toLocaleString()}</b>.</p><br><strong>Active Subject:</strong> ${activeSubject}`;
                const sendAt = new Date(now.getTime() + 1 * 60000); // 1 minute later
